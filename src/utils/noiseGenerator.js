let audioContext = null;
let noiseNode = null;
let gainNode = null;
let filterNode = null;

const initializeAudioContext = () => {
  if (!audioContext) {
    audioContext = new (window.AudioContext || window.webkitAudioContext)();
  }
  if (audioContext.state === "suspended") {
    audioContext.resume();
  }
};

export const createNoiseAudio = (type, volume) => {
  try {
    initializeAudioContext();
    const currentAudioContext = audioContext;

    const bufferSize = currentAudioContext.sampleRate * 2;
    const noiseBuffer = currentAudioContext.createBuffer(
      1,
      bufferSize,
      currentAudioContext.sampleRate
    );
    const output = noiseBuffer.getChannelData(0);

    if (type === "white") {
      for (let i = 0; i < bufferSize; i++) {
        output[i] = Math.random() * 2 - 1;
      }
    } else if (type === "pink") {
      let b0 = 0,
        b1 = 0,
        b2 = 0,
        b3 = 0,
        b4 = 0,
        b5 = 0,
        b6 = 0;
      for (let i = 0; i < bufferSize; i++) {
        const white = Math.random() * 2 - 1;
        b0 = 0.99886 * b0 + white * 0.0555179;
        b1 = 0.99332 * b1 + white * 0.0750759;
        b2 = 0.969 * b2 + white * 0.153852;
        b3 = 0.8665 * b3 + white * 0.3104856;
        b4 = 0.55 * b4 + white * 0.5329522;
        b5 = -0.7616 * b5 - white * 0.016898;
        output[i] = b0 + b1 + b2 + b3 + b4 + b5 + b6 + white * 0.5362;
        output[i] *= 0.11;
        b6 = white * 0.115926;
      }
    } else if (type === "brown") {
      let lastOut = 0;
      for (let i = 0; i < bufferSize; i++) {
        const white = Math.random() * 2 - 1;
        output[i] = (lastOut + 0.02 * white) / 1.02;
        lastOut = output[i];
        output[i] *= 3.5;
      }
    } else if (type === "airplane_cabin") {
      let b0 = 0,
        b1 = 0,
        b2 = 0,
        b3 = 0,
        b4 = 0,
        b5 = 0,
        b6 = 0;
      for (let i = 0; i < bufferSize; i++) {
        const white = Math.random() * 2 - 1;
        b0 = 0.99886 * b0 + white * 0.0555179;
        b1 = 0.99332 * b1 + white * 0.0750759;
        b2 = 0.969 * b2 + white * 0.153852;
        b3 = 0.8665 * b3 + white * 0.3104856;
        b4 = 0.55 * b4 + white * 0.5329522;
        b5 = -0.7616 * b5 - white * 0.016898;
        output[i] = b0 + b1 + b2 + b3 + b4 + b5 + b6 + white * 0.5362;
        output[i] *= 0.11;
        b6 = white * 0.115926;
      }
    }

    const source = currentAudioContext.createBufferSource();
    source.buffer = noiseBuffer;
    source.loop = true;

    const currentGainNode = currentAudioContext.createGain();
    currentGainNode.gain.value = volume;

    if (type === "airplane_cabin") {
      const airplaneCabinFilter = currentAudioContext.createBiquadFilter();
      airplaneCabinFilter.type = "lowpass";
      airplaneCabinFilter.frequency.value = 800;
      airplaneCabinFilter.Q.value = 1;

      source.connect(airplaneCabinFilter);
      airplaneCabinFilter.connect(currentGainNode);
      filterNode = airplaneCabinFilter;
    } else {
      source.connect(currentGainNode);
    }

    currentGainNode.connect(currentAudioContext.destination);

    noiseNode = source;
    gainNode = currentGainNode;

    return { noiseNode, gainNode, filterNode };
  } catch (error) {
    console.error("Error in noise creation:", error);
    return { noiseNode: null, gainNode: null, filterNode: null };
  }
};

export const stopNoise = () => {
  if (noiseNode) {
    try {
      noiseNode.stop();
      noiseNode = null;
      filterNode = null;
    } catch (error) {
      console.error("Error stopping noise:", error);
    }
  }
};

export const setNoiseVolume = (newVolume) => {
  if (gainNode) {
    gainNode.gain.value = newVolume;
  }
};

export const setAirplaneCabinFilterFrequency = (newFrequency) => {
  if (filterNode && filterNode.type === "lowpass") {
    filterNode.frequency.value = newFrequency;
  }
};

export const setAirplaneCabinFilterQ = (newQ) => {
  if (filterNode && filterNode.type === "lowpass") {
    filterNode.Q.value = newQ;
  }
};

export const closeAudioContext = () => {
  if (audioContext && audioContext.state !== "closed") {
    audioContext.close();
    audioContext = null;
  }
};
